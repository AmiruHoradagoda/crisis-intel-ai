...
class Config:
    """Configuration container with dot-separated key lookup."""
...
def get_strong_techniques() -> list[str]:
    """Get techniques that trigger strong model routing."""
    return get_config().get(
        "models.strong_techniques",
        ["strong", "complex", "advanced"],
    )


def get_general_techniques() -> list[str]:
    return get_config().get(
        "models.general_techniques",
        ["general", "zero_shot", "few_shot"],
    )
