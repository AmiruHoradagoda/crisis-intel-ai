...
class LLMClient:
    ...
    def __init__(
        ...
        Args:
            ...
            hard_prompt_cap: Optional hard limit on prompt tokens (triggers truncation)
        """
        ...
        self.backoff_base = (
            backoff_base if backoff_base is not None else get_backoff_base()
        )
        self.backoff_jitter = (
            backoff_jitter if backoff_jitter is not None else get_backoff_jitter()
        )
    ...
    def chat(
        ...
        **kwargs,
        ...
        Args:
            ...
            temperature: Sampling temperature (None uses configuration)
            max_tokens: Max completion tokens (None uses configuration)
            task_type: Optional task category for configured defaults
        ...
        if temperature is None:
            temperature = get_default_temperature(task_type)
        if max_tokens is None:
            max_tokens = get_default_max_tokens(task_type)
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # Retrying the same oversized prompt cannot resolve overflow
